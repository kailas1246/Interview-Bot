class InterviewBot {
  ...
  bindEvents() {
    ...
    document.addEventListener("keydown", (event) => {
      if (
        event.key === "Escape" &&
        this.sessionId &&
        this.interviewInterface.classList.contains("d-none") === false
      ) {
        this.exitInterview();
      }
    });
  ...
  async skipQuestion() {
    ...
    try {
      ...
      this.skipQuestionBtn.innerHTML =
      ...
      this.showError("Failed to skip question: " + error.message);
    } finally {
  ...
  updateListenButton() {
    if (this.isListening) {
      ...
      this.listenBtn.innerHTML =
        '<i class="fas fa-stop me-2"></i><span>Stop Listening</span>';
    } else {
      this.listenBtn.className = "btn btn-success btn-lg";
      this.listenBtn.innerHTML =
        '<i class="fas fa-microphone me-2"></i><span>Start Speaking</span>';
    }
  }
  ...
  async submitAnswer() {
    ...
    try {
      this.submitAnswerBtn.disabled = true;
      this.submitAnswerBtn.innerHTML =
  ...
  displayFeedback(feedback, score, isSatisfactory, issues, suggestions) {
    ...
    const satisfactoryClass = isSatisfactory
      ? "alert-success"
      : "alert-warning";
    const satisfactoryIcon = isSatisfactory
      ? "fa-check-circle"
  ...
  speakRetryMessage(message) {
    const utterance = new SpeechSynthesisUtterance(
      message + " " + this.questionText.textContent
    );
    utterance.rate = 0.8;
    this.synthesis.speak(utterance);
  }
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <h6 class="card-title mb-0">Question ${
                              index + 1
                            }</h6>
